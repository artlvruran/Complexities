    ...
    def solve(self, iterations=500):
        ...
        for _ in range(iterations):
            for i, particle in enumerate(self.particles):
                ...
                # new_position, new_cost = self.cross(cur_position, particle.pbest)
                ...
                # new_position, new_cost = self.cross(cur_position, self.best_position)
                new_position = self.deterministic_crossover(cur_position, self.best_position)
                new_cost = self.calc_cost(new_position)
                if new_cost < cur_cost or np.random.rand() < self.c2:
                    cur_position = new_position
                    cur_cost = new_cost

                # cur_position, cur_cost = self.mutate(cur_position)
