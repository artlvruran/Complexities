    ...
    def solve(self, iterations=500) -> Output:
        ...
        for _ in range(iterations):
            ...
            for i, particle in enumerate(self.particles):
                ...
                left = self.particles[(i - 1) % self.particle_count]
                right = self.particles[(i + 1) % self.particle_count]

                new_position = self.probabilistic_crossover(cur_position, particle.pbest)
                new_cost = self.calc_cost(new_position)

                if new_cost < self.particles[i].pbest_cost:
                    self.particles[i].pbest_cost = new_cost
                    self.particles[i].pbest = Position(new_position.route)
                if new_cost < cur_cost or np.random.rand() < self.c1:
                    cur_position = new_position
                    cur_cost = new_cost

                candidates = [left.pbest, right.pbest]
                costs = [left.pbest_cost, right.pbest_cost]
                lbest = candidates[int(np.argmin(costs))]
                new_position = self.deterministic_crossover(cur_position, lbest)
                new_cost = self.calc_cost(new_position)
                if new_cost < self.particles[i].pbest_cost:
                    self.particles[i].pbest_cost = new_cost
                    self.particles[i].pbest = Position(new_position.route)
                ...
                new_position = self.directional_mutation(cur_position)
                new_cost = self.calc_cost(new_position)
                if new_cost < self.particles[i].pbest_cost:
                    self.particles[i].pbest_cost = new_cost
                    self.particles[i].pbest = Position(new_position.route)
